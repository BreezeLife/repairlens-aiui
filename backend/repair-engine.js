...
export function normalizeModelPlan(payload, input = {}) {
  ...
  const requestedEquipment = text(input.equipment, DEFAULT_EQUIPMENT, 120);
  if (typeof raw.equipment === 'string' && raw.equipment.trim()
    && raw.equipment.trim().toLowerCase() !== requestedEquipment.toLowerCase()) {
    throw new Error('Model plan targets a different equipment case');
  }
